...
def createmarque(request):
    try:
        ...
        reps=Represent.objects.filter(pk__in=commercialexcluded)"""
        Mark.objects.create(name=name, image=image)
...
def createclientaccount2(request):
    print('>>>>>>')
    clientcode=request.GET.get('clientcode')
    client= Client.objects.get(code=clientcode)
    olduser=client.user
...
def deactivateclientaccount(request):
...
def activateclientaccount(request):
    code=request.GET.get('code')
    client=Client.objects.get(code=code)
    user=client.user
    user.is_active=True
...
def createnewclientaccount2(request):
    clientcode=request.GET.get('clientcode')
    client= Client.objects.get(code=clientcode)
    olduser=client.user
    username=request.GET.get('username')
...
def createnewrepaccount2(request):
    repid=request.GET.get('repid')
    rep= Represent.objects.get(pk=repid)
    olduser=rep.user
